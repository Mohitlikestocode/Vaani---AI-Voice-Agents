...
const cases = [
  ...
  },
  {
    ...
    name: "Salons & Spas",
    icon: Scissors,
    desc: "Fill your calendar around the clock. Swara books, reschedules and upsells while your stylists stay with the client in the chair.",
    bullets: ["Books & reschedules appointments", "Explains services & pricing clearly", "Upsells packages automatically"],
    stat: { value: "3Ã—", label: "more bookings after hours" },
  ...
  },
  {
    ...
    icon: Stethoscope,
    desc: "A calm, always-available front desk. Swara shares hours, books appointments and answers patient FAQs in their own language.",
  ...
  },
  {
    ...
    icon: Hotel,
    desc: "A 24/7 concierge that never sleeps — Swara takes bookings, handles room service and recommends the best spots in town.",
  ...
  },
  {
    ...
    name: "Tech Support",
    icon: Headset,
    desc: "First-line support without the hold music. Swara troubleshoots common issues instantly and escalates the rest with full context.",
...
export const UseCases = () => {
  ...
  return (
    <section id="use-cases" data-testid="use-cases" className="bg-swara-sand px-6 py-28 md:px-10 md:py-40">
            ...
            return (
              <button
                ...
                className={`group flex items-center gap-2 rounded-full border px-4 py-2.5 text-sm font-medium transition-all duration-300 ${
                  isActive
                    ? "border-swara-ink bg-swara-ink text-white"
                    : "border-zinc-200 bg-white text-swara-muted hover:border-swara-ink/30 hover:text-swara-ink"
                }`}
              >
                <Icon className="h-4 w-4" strokeWidth={2} />
                {item.tab}
                {item.popular && (
                  <span className={`rounded-full px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wide ${isActive ? "bg-swara-gold text-white" : "bg-swara-gold/10 text-swara-gold"}`}>
                    Popular
                  </span>
                )}
              </button>
            ...
            <div className="grid lg:grid-cols-[1fr_1.05fr]">
              ...
              <div className="flex flex-col justify-center p-8 md:p-14">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <span className="font-mono text-[11px] uppercase tracking-[0.25em] text-swara-gold">
                      Use case {String(active + 1).padStart(2, "0")}
                    </span>
                    <h3 className="mt-4 font-display text-3xl font-semibold tracking-tight text-swara-ink md:text-4xl">
                      {c.name}
                    </h3>
                    <p className="mt-4 max-w-md text-swara-muted">{c.desc}</p>

                    <ul className="mt-7 space-y-3">
                      {c.bullets.map((b) => (
                        <li key={b} className="flex items-start gap-3 text-sm text-swara-ink">
                          <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-swara-gold/10">
                            <Check className="h-3 w-3 text-swara-gold" strokeWidth={3} />
                          </span>
                          {b}
                        </li>
                      ))}
                    ...
                    <div className="mt-8 inline-flex items-center gap-3 rounded-2xl border border-zinc-200 bg-swara-sand/60 px-5 py-3">
                      <TrendingUp className="h-5 w-5 text-swara-gold" />
                      <span className="font-display text-2xl font-semibold text-swara-ink">{c.stat.value}</span>
                      <span className="text-sm text-swara-muted">{c.stat.label}</span>
                    </div>
                  </motion.div>
                </AnimatePresence>
              </div>
              ...
              <div className="relative min-h-[420px] overflow-hidden bg-swara-ink lg:min-h-full">
                <AnimatePresence mode="wait">
                  <motion.div
                    key={c.key}
                  ...
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-swara-gold/30 via-swara-ink to-swara-ink" />
                    <img
                      src={c.image}
                      alt={c.name}
                      onError={(e) => {
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-swara-ink/90 via-swara-ink/20 to-transparent" />
                  </motion.div>
                </AnimatePresence>
                ...
                <div className="absolute left-6 top-6 z-10 flex items-center gap-2 rounded-full bg-white/10 px-3.5 py-1.5 backdrop-blur-md">
                  ...
                  <span className="font-mono text-[10px] uppercase tracking-wider text-white">Swara · live</span>
                </div>
                        ...
                        <div key={k} className={`flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
                          <span
                            className={`max-w-[85%] rounded-2xl px-3.5 py-2 text-sm leading-snug ${
                              m.from === "user"
                                ? "rounded-br-sm bg-swara-gold text-white"
